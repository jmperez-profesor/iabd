...
El primer paso es cargar los archivos de entrenamiento y validación desde URLs remotas. Utilizaremos la función `load_dataset()` de la librería Hugging Face Datasets, indicando que queremos cargar archivos JSON alojados en GitHub. Los datos remotos se corresponden con `train-v2.0-es.json` (entrenamiento) y `dev-v2.0-es.json` (validación).
